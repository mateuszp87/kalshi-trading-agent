...
SIGNAL_LOG_PATH = os.environ.get("SIGNAL_LOG_PATH", "/data/signal_log.json")
...
class KalshiTradingAgent:
    ...
    async def _place(self, client, market: KalshiMarket, side: str,
        ...
        # ── COHERENCE GATE ──────────────────────────────────────────────
        # Refuse any bet where the signed edge on the side we're actually
        # buying is not positive. signal.edge is (p_model - mid) from the YES
        # frame, so buying YES needs edge>0, buying NO needs edge<0. This is
        # the catch-all for a category builder (e.g. weather ensemble) emitting
        # a recommendation inconsistent with its own probability. Kelly uses
        # abs(edge) downstream, which would otherwise size a wrong-side bet big.
        _signed_edge = signal.edge if side == "yes" else -signal.edge
        if _signed_edge <= 0:
            log.info(f"  \u2298 COHERENCE SKIP {market.ticker} "
                     f"{side.upper()}: signed_edge={_signed_edge:+.3f} "
                     f"(edge={signal.edge:+.3f}) \u2014 wrong side of model, blocked")
            return False
